...
def largest_indices(ary: np.ndarray, n: int):
    ...
    return zip(*np.unravel_index(indices, ary.shape))
...
def display_possible(*drones_days_counts: np.ndarray, days: Iterable[int]=range(1,26)):
    for day in days:
        ...
        for drone_days_counts in drones_days_counts:
            ...
            data += drone_day_counts / drone_day_counts.sum()
        
        data = np.pad(data / len(drones_days_counts), ((1, 0), (1, 0))) * 100
        ...
        ax.format_coord = lambda x, y: old_fmt(round(x), round(y))
